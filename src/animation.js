...
import { executeMove, CELLS } from './puzzle.js';
...
export class AnimationEngine {
  ...
  tick(timestamp, cubies, centralCellIndex) {
    ...
    if (a.type === 'move') {
      const R = mat4PlaneRotation(a.planeA, a.planeB, a.totalAngle * t);
      const getState = (i) => {
        const pre = a.pre.get(i);
        if (!pre) return null;                       // unaffected → committed state
        return {
          center4: mat4MulVec4(R, pre.center),
          orient:  mat4Mul(R, pre.orient),
        };
      };
      return { type: 'move', getState, cellIndex: a.cellIndex, R };
    }
  ...
  _finishActive() {
    const a = this.active;
    if (a.type === 'move' && this.onMoveComplete) this.onMoveComplete(a.descriptor);
